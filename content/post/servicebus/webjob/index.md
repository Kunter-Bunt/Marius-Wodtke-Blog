...
In [the first post](/post/servicebus/exporting-events) of this series we started with sending Dataverse Events to a Service Bus. Of course that in it self does not have any value, we need to do something with these Events!
...
Once everthing is configured we build and run the HostBuilder. This `RunAsync()` will block the console indefinitely and usually it throws here if you misconfigured something.
...
Ok, over to the Functions.cs. As the name suggests, this is the Entrypoint for our functionality. 
```
...
```
Technicially these are only 2 lines of code, but there is a lot going. I'll start with the `ServiceBusTriggerAttribute`, the first parameter ("dataverse") here is the topic, the second one ("account-export") is the subscription. The Connection parameter ("AzureServiceBus") is actually just a reference to the settings, so a quick dive into the appsettings.json file.
```
{
  "AzureServiceBus": {
...
And now we lift the secret how the authentication works! So the WebJob SDK will automatically use the Managed Identity we just assigned, so the only thing we need to make sure is that this Managed Identity is authorized to access the Service Bus. This is done in the IAM Section of the Service Bus.
...
## The result
Now its time for a debug run, but well, there is not much logic in my function. It just prints the `MessageName` to prove that the deserilization works. Note the interfaces implemented by `RemoteExecutionContext`, it does implement the `IPluginExecutionContext` that you will already know from regular plugins. So everything as expected, the `InputParameters` will contain the "Target", if you have configured a PreImage it will be in the `PreEntityImages` and so on.

![](Result.png)

## Deploying
After the successful debug lets deploy. While later you should look into Pipelines with Github or Azure DevOps, a first deployment (or publish) can happen absolutely happen directly from Visual Studio.
...
![And that publish should end up with a sucess](Publish6.png)
